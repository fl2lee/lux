// Copyright (c) 2017 The Luxcoin developers
...
// The main object for accessing Luxsend
CLuxsendPool LuxsendPool;
// A helper object for signing messages from Masternodes
CLuxsendSigner LuxsendSigner;
// The current Luxsends in progress on the network
std::vector<CLuxsendQueue> vecLuxsendQueue;
// Keep track of the used Masternodes
std::vector<CTxIn> vecMasternodesUsed;
// keep track of the scanning errors I've seen
map<uint256, CLuxsendBroadcastTx> mapLuxsendBroadcastTxes;
...
void CLuxsendPool::ProcessMessageLuxsend(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    if(fLiteMode) return; //disable all Luxsend/Masternode related functionality
    if(!IsBlockchainSynced()) return;

    if (strCommand == "dsa") { //Luxsend Accept Into Pool
    ...
    } else if (strCommand == "dsq") { //Luxsend Queue
        TRY_LOCK(cs_Luxsend, lockRecv);
        if(!lockRecv) return;
        ...
        if(dsq.ready) {
            ...
            if(state == POOL_STATUS_QUEUE){
                LogPrintf("Luxsend queue is ready - %s\n", addr.ToString().c_str());
                PrepareLuxsendDenominate();
            }
        } else {
            ...
            LogPrint("Luxsend", "dsq last %d last2 %d count %d\n", pmn->nLastDsq, pmn->nLastDsq + mnodeman.size()/5, mnodeman.nDsqCount);
            //don't allow a few nodes to dominate the queuing process
            if(pmn->nLastDsq != 0 &&
                pmn->nLastDsq + mnodeman.CountMasternodesAboveProtocol(MIN_POOL_PEER_PROTO_VERSION)/5 > mnodeman.nDsqCount){
                LogPrint("Luxsend", "dsq -- Masternode sending too many dsq messages. %s \n", pmn->addr.ToString().c_str());
                return;
            }
            mnodeman.nDsqCount++;
            ...
            LogPrint("Luxsend", "dsq - new Luxsend queue object - %s\n", addr.ToString().c_str());
            vecLuxsendQueue.push_back(dsq);
            dsq.Relay();
            dsq.time = GetTime();
        }

    } else if (strCommand == "dsi") { //Luxsend vIn
        std::string error = "";
        if (pfrom->nVersion < MIN_POOL_PEER_PROTO_VERSION) {
        ...
        //check it like a transaction
        {
            ...
            if (nValueIn > Luxsend_POOL_MAX) {
                LogPrintf("dsi -- more than Luxsend pool max! %s\n", tx.ToString().c_str());
                error = _("Value more than Luxsend pool maximum allows.");
                pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), MASTERNODE_REJECTED, error);
                return;
            }
    ...
    } else if (strCommand == "dssu") { //Luxsend status update
        if (pfrom->nVersion < MIN_POOL_PEER_PROTO_VERSION) {
            return;
        }
        if(!pSubmittedToMasternode) return;
        ...
        LogPrint("Luxsend", "dssu - state: %i entriesCount: %i accepted: %i error: %s \n", state, entriesCount, accepted, error.c_str());

        if((accepted != 1 && accepted != 0) && sessionID != sessionIDMessage){
            LogPrintf("dssu - message doesn't match current Luxsend session %d %d\n", sessionID, sessionIDMessage);
            return;
        }

        StatusUpdate(state, entriesCount, accepted, error, sessionIDMessage);

    } else if (strCommand == "dss") { //Luxsend Sign Final Tx
        if (pfrom->nVersion < MIN_POOL_PEER_PROTO_VERSION) {
            return;
        }
        ...
        BOOST_FOREACH(const CTxIn item, sigs)
        {
            ...
            LogPrint("Luxsend", " -- sigs count %d %d\n", (int)sigs.size(), count);
            count++;
        }

        if(success){
        LuxsendPool.Check();
            RelayStatus(LuxsendPool.sessionID, LuxsendPool.GetState(), LuxsendPool.GetEntriesCount(), MASTERNODE_RESET);
        }
    } else if (strCommand == "dsf") { //Luxsend Final tx
        if (pfrom->nVersion < MIN_POOL_PEER_PROTO_VERSION) {
            return;
        }
        if(!pSubmittedToMasternode) return;
        ...
        if(sessionID != sessionIDMessage){
            LogPrint("Luxsend", "dsf - message doesn't match current Luxsend session %d %d\n", sessionID, sessionIDMessage);
            return;
        }
        //check to see if input is spent already? (and probably not confirmed)
        SignFinalTransaction(txNew, pfrom);
    } else if (strCommand == "dsc") { //Luxsend Complete
        if (pfrom->nVersion < MIN_POOL_PEER_PROTO_VERSION) {
            return;
        }
        ...
        if(sessionID != sessionIDMessage){
            LogPrint("Luxsend", "dsc - message doesn't match current Luxsend session %d %d\n", LuxsendPool.sessionID, sessionIDMessage);
            return;
        }

        LuxsendPool.CompletedTransaction(error, errorID);
    }
...
bool CLuxsendPool::SetCollateralAddress(std::string strAddress){
    CLuxcoinAddress address;
    if (!address.SetString(strAddress))
    {
        LogPrintf("CLuxsendPool::SetCollateralAddress - Invalid Luxsend collateral address\n");
        return false;
    }
...
//
// Unlock coins after Luxsend fails or succeeds
//
void CLuxsendPool::UnlockCoins(){
    while(true) {
...
bool CLuxsendPool::IsBlockchainSynced()
{
...
std::string CLuxsendPool::GetStatus()
{
    static int showingLuxsendMessage = 0;
    showingLuxsendMessage+=10;
    std::string suffix = "";
    ...
    switch(state) {
        case POOL_STATUS_IDLE:
            return _("Luxsend is idle.");
        case POOL_STATUS_ACCEPTING_ENTRIES:
            if(entriesCount == 0) {
                showingLuxsendMessage = 0;
                return strAutoDenomResult;
            } else if (lastEntryAccepted == 1) {
                if(showingLuxsendMessage % 10 > 8) {
                    lastEntryAccepted = 0;
                    showingLuxsendMessage = 0;
                }
                return _("Luxsend request complete:") + " " + _("Your transaction was accepted into the pool!");
            } else {
                std::string suffix = "";
                if(     showingLuxsendMessage % 70 <= 40) return strprintf(_("Submitted following entries to masternode: %u / %d"), entriesCount, GetMaxPoolTransactions());
                else if(showingLuxsendMessage % 70 <= 50) suffix = ".";
                else if(showingLuxsendMessage % 70 <= 60) suffix = "..";
                else if(showingLuxsendMessage % 70 <= 70) suffix = "...";
        ...
        case POOL_STATUS_SIGNING:
            if(     showingLuxsendMessage % 70 <= 40) return _("Found enough users, signing ...");
            else if(showingLuxsendMessage % 70 <= 50) suffix = ".";
            else if(showingLuxsendMessage % 70 <= 60) suffix = "..";
            else if(showingLuxsendMessage % 70 <= 70) suffix = "...";
        ...
        case POOL_STATUS_ERROR:
            return _("Luxsend request incomplete:") + " " + lastMessage + " " + _("Will retry...");
        case POOL_STATUS_SUCCESS:
            return _("Luxsend request complete:") + " " + lastMessage;
        case POOL_STATUS_QUEUE:
            if(     showingLuxsendMessage % 70 <= 30) suffix = ".";
            else if(showingLuxsendMessage % 70 <= 50) suffix = "..";
            else if(showingLuxsendMessage % 70 <= 70) suffix = "...";
...
//
// Check the Luxsend progress and send client updates if a Masternode
//
void CLuxsendPool::Check()
{
    if(fMasterNode) LogPrint("Luxsend", "CLuxsendPool::Check() - entries count %lu\n", entries.size());
    //printf("CLuxsendPool::Check() %d - %d - %d\n", state, anonTx.CountEntries(), GetTimeMillis()-lastTimeChanged);

    if(fMasterNode) {
        LogPrint("Luxsend", "CLuxsendPool::Check() - entries count %lu\n", entries.size());
        // If entries is full, then move on to the next phase
        if(state == POOL_STATUS_ACCEPTING_ENTRIES && (int)entries.size() >= GetMaxPoolTransactions())
        {
            LogPrint("Luxsend", "CLuxsendPool::Check() -- TRYING TRANSACTION \n");
    ...
    if(state == POOL_STATUS_FINALIZE_TRANSACTION) {
        LogPrint("Luxsend", "CLuxsendPool::Check() -- FINALIZE TRANSACTIONS\n");
        ...
        if (fMasterNode) {
            ...
            LogPrint("Luxsend", "Transaction 1: %s\n", txNew.ToString());
    ...
    if(fMasterNode && state == POOL_STATUS_SIGNING && SignaturesComplete()) {
        LogPrint("Luxsend", "CLuxsendPool::Check() -- SIGNING\n");
    ...
    if((state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS) && GetTimeMillis()-lastTimeChanged >= 10000) {
        LogPrint("Luxsend", "CLuxsendPool::Check() -- timeout, RESETTING\n");
        UnlockCoins();
        SetNull();
...
void CLuxsendPool::CheckFinalTransaction()
{
    ...
    {
        LogPrint("Luxsend", "Transaction 2: %s\n", txNew.ToString());

        // See if the transaction is valid
        if (!txNew.AcceptToMemoryPool(false, true, true))
        {
            LogPrintf("CLuxsendPool::Check() - CommitTransaction : Error: Transaction not valid\n");
            SetNull();
        ...
        LogPrintf("CLuxsendPool::Check() -- IS MASTER -- TRANSMITTING Luxsend\n");
        ...
        if(!LuxsendSigner.SetKey(strMasterNodePrivKey, strError, key2, pubkey2))
        {
            LogPrintf("CLuxsendPool::Check() - ERROR: Invalid Masternodeprivkey: '%s'\n", strError);
            return;
        }

        if(!LuxsendSigner.SignMessage(strMessage, strError, vchSig, key2)) {
            LogPrintf("CLuxsendPool::Check() - Sign message failed\n");
            return;
        }

        if(!LuxsendSigner.VerifyMessage(pubkey2, vchSig, strMessage, strError)) {
            LogPrintf("CLuxsendPool::Check() - Verify message failed\n");
            return;
        }

        string txHash = txNew.GetHash().ToString().c_str();
        LogPrintf("CLuxsendPool::Check() -- txHash %d \n", txHash);
        if(!mapLuxsendBroadcastTxes.count(txNew.GetHash())){
            CLuxsendBroadcastTx dstx;
            dstx.tx = txNew;
            dstx.vin = activeMasternode.vin;
            dstx.vchSig = vchSig;
            dstx.sigTime = sigTime;

            mapLuxsendBroadcastTxes.insert(make_pair(txNew.GetHash(), dstx));
        }
        ...
        // Reset
        LogPrint("Luxsend", "CLuxsendPool::Check() -- COMPLETED -- RESETTING \n");
        SetNull();
        RelayStatus(sessionID, GetState(), GetEntriesCount(), MASTERNODE_RESET);
    }
}
...
//
// Why bother? Luxsend uses collateral to ensure abuse to the process is kept to a minimum.
// The submission and signing stages in Luxsend are completely separate. In the cases where
...
void CLuxsendPool::ChargeFees(){
    ...
    if(state == POOL_STATUS_ACCEPTING_ENTRIES){
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            bool found = false;
            BOOST_FOREACH(const CLuxsendEntry& v, entries) {
            ...
            if(!found){
                LogPrintf("CLuxsendPool::ChargeFees -- found uncooperative node (didn't send transaction). Found offence.\n");
                offences++;
            }
        }
    }
    ...
    if(state == POOL_STATUS_SIGNING) {
        ...
        BOOST_FOREACH(const CLuxsendEntry v, entries) {
            BOOST_FOREACH(const CTxDSIn s, v.sev) {
                if(!s.fHasSig){
                    LogPrintf("CLuxsendPool::ChargeFees -- found uncooperative node (didn't sign). Found offence\n");
                    offences++;
                }
            }
        }
    ...
    if(state == POOL_STATUS_ACCEPTING_ENTRIES){
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            // This queue entry didn't send us the promised transaction
            if(!found && r > target){
                LogPrintf("CLuxsendPool::ChargeFees -- found uncooperative node (didn't send transaction). charging fees.\n");
                ...
                if (!wtxCollateral.AcceptToMemoryPool(true))
                {
                    ...
                    LogPrintf("CLuxsendPool::ChargeFees() : Error: Transaction not valid");
                }
        ...
        BOOST_FOREACH(const CLuxsendEntry v, entries) {
            BOOST_FOREACH(const CTxDSIn s, v.sev) {
                if(!s.fHasSig && r > target){
                    LogPrintf("CLuxsendPool::ChargeFees -- found uncooperative node (didn't sign). charging fees.\n");
...
//  - Luxsend is completely free, to pay miners we randomly pay the collateral of users.
void CLuxsendPool::ChargeRandomFees(){
    if(fMasterNode) {
        int i = 0;
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                Being that Luxsend has "no fees" we need to have some kind of cost associated
                with using it to stop abuse. Otherwise it could serve as an attack vector and
                allow endless transaction that would bloat Luxcoin and make it unusable. To
            ...
            */
            if(r <= 10)
            {
                LogPrintf("CLuxsendPool::ChargeRandomFees -- charging random fees. %u\n", i);
                ...
                {
                    // This must not fail. The transaction has already been signed and recorded.
                    LogPrintf("CLuxsendPool::ChargeRandomFees() : Error: Transaction not valid");
                }
                wtxCollateral.RelayWalletTransaction();
            }
        }
...
//
// Check for various timeouts (queue objects, Luxsend, etc)
//
void CLuxsendPool::CheckTimeout(){
    if(!fEnableLuxsend && !fMasterNode) return;

    // catching hanging sessions
    if(!fMasterNode) {
        switch(state) {
            case POOL_STATUS_TRANSMISSION:
                LogPrint("Luxsend", "CLuxsendPool::CheckTimeout() -- Session complete -- Running Check()\n");
                Check();
                break;
            case POOL_STATUS_ERROR:
                LogPrint("Luxsend", "CLuxsendPool::CheckTimeout() -- Pool error -- Running Check()\n");
                Check();
                break;
            case POOL_STATUS_SUCCESS:
                LogPrint("Luxsend", "CLuxsendPool::CheckTimeout() -- Pool success -- Running Check()\n");
                Check();
                break;
        }
    }

    // check Luxsend queue objects for timeouts
    int c = 0;
    vector<CLuxsendQueue>::iterator it = vecLuxsendQueue.begin();
    while(it != vecLuxsendQueue.end()){
        if((*it).IsExpired()){
            LogPrint("Luxsend", "CLuxsendPool::CheckTimeout() : Removing expired queue entry - %d\n", c);
            it = vecLuxsendQueue.erase(it);
        } else ++it;
        c++;
    }
    ...
    if(state == POOL_STATUS_ACCEPTING_ENTRIES || state == POOL_STATUS_QUEUE){
        ...
        vector<CLuxsendEntry>::iterator it2 = entries.begin();
        while(it2 != entries.end()){
            if((*it2).IsExpired()){
                LogPrint("Luxsend", "CLuxsendPool::CheckTimeout() : Removing expired entry - %d\n", c);
        ...
        if(GetTimeMillis()-lastTimeChanged >= (Luxsend_QUEUE_TIMEOUT*1000)+addLagTime){
            UnlockCoins();
            SetNull();
        }
    } else if(GetTimeMillis()-lastTimeChanged >= (Luxsend_QUEUE_TIMEOUT*1000)+addLagTime){
        LogPrint("Luxsend", "CLuxsendPool::CheckTimeout() -- Session timed out (%ds) -- resetting\n", Luxsend_QUEUE_TIMEOUT);
        UnlockCoins();
        SetNull();
    ...
    if(state == POOL_STATUS_SIGNING && GetTimeMillis()-lastTimeChanged >= (Luxsend_SIGNING_TIMEOUT*1000)+addLagTime ) {
            LogPrint("Luxsend", "CLuxsendPool::CheckTimeout() -- Session timed out (%ds) -- restting\n", Luxsend_SIGNING_TIMEOUT);
            ChargeFees();
            UnlockCoins();
            SetNull();
...
//
void CLuxsendPool::CheckForCompleteQueue(){
    if(!fEnableLuxsend && !fMasterNode) return;
...
bool CLuxsendPool::SignatureValid(const CScript& newSig, const CTxIn& newVin){
    CTransaction txNew;
    txNew.vin.clear();
    txNew.vout.clear();
    ...
    BOOST_FOREACH(CLuxsendEntry& e, entries) {
    ...
    if(found >= 0){ //might have to do this one input at a time?
        ...
        LogPrint("Luxsend", "CLuxsendPool::SignatureValid() - Sign with sig %s\n", newSig.ToString().substr(0,24));
        if (!VerifyScript(txNew.vin[n].scriptSig, sigPubKey, txNew, n, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC, 0)){
            LogPrint("Luxsend", "CLuxsendPool::SignatureValid() - Signing - Error signing input %u\n", n);
            return false;
        }
    }

    LogPrint("Luxsend", "CLuxsendPool::SignatureValid() - Signing - Successfully validated input\n");
    return true;
}

// check to make sure the collateral provided by the client is valid
bool CLuxsendPool::IsCollateralValid(const CTransaction& txCollateral){
    ...
    BOOST_FOREACH(const CTxOut o, txCollateral.vout){
        ...
        if(!o.scriptPubKey.IsNormalPaymentScript()){
            LogPrintf ("CLuxsendPool::IsCollateralValid - Invalid Script %s\n", txCollateral.ToString());
            return false;
        }
    }
    ...
    if(missingTx){
        LogPrint("Luxsend", "CLuxsendPool::IsCollateralValid - Unknown inputs in collateral transaction - %s\n", txCollateral.ToString());
        return false;
    }

    //collateral transactions are required to pay out Luxsend_COLLATERAL as a fee to the miners
    if(nValueIn-nValueOut < Luxsend_COLLATERAL) {
        LogPrint("Luxsend", "CLuxsendPool::IsCollateralValid - did not include enough fees in transaction %d\n%s\n", nValueOut-nValueIn, txCollateral.ToString());
        return false;
    }

    LogPrint("Luxsend", "CLuxsendPool::IsCollateralValid %s\n", txCollateral.ToString());

    {
        LOCK(cs_main);
        CValidationState state;
        if(!AcceptableInputs(mempool, txCollateral, true, NULL)){
            LogPrintf ("CLuxsendPool::IsCollateralValid - didn't pass IsAcceptable\n");
            return false;
        }
    }
...
//
bool CLuxsendPool::AddEntry(const std::vector<CTxIn>& newInput, const int64_t& nAmount, const CTransaction& txCollateral, const std::vector<CTxOut>& newOutput, std::string& error){
    if (!fMasterNode) return false;

    BOOST_FOREACH(CTxIn in, newInput) {
        if (in.prevout.IsNull() || nAmount < 0) {
            LogPrint("Luxsend", "CLuxsendPool::AddEntry - input not valid!\n");
    ...
    if (!IsCollateralValid(txCollateral)){
        LogPrint("Luxsend", "CLuxsendPool::AddEntry - collateral not valid!\n");
    ...
    if((int)entries.size() >= GetMaxPoolTransactions()){
        LogPrint("Luxsend", "CLuxsendPool::AddEntry - entries is full!\n");
    ...
    BOOST_FOREACH(CTxIn in, newInput) {
        LogPrint("Luxsend", "looking for vin -- %s\n", in.ToString());
        BOOST_FOREACH(const CLuxsendEntry& v, entries) {
            BOOST_FOREACH(const CTxDSIn& s, v.sev){
                if((CTxIn)s == in) {
                    LogPrint("Luxsend", "CLuxsendPool::AddEntry - found in vin\n");
    ...
    LogPrint("Luxsend", "CLuxsendPool::AddEntry -- adding %s\n", newInput[0].ToString());
    error = "";

    return true;
}

bool CLuxsendPool::AddScriptSig(const CTxIn& newVin){
    LogPrint("Luxsend", "CLuxsendPool::AddScriptSig -- new sig  %s\n", newVin.scriptSig.ToString().substr(0,24));

    BOOST_FOREACH(const CLuxsendEntry& v, entries) {
        BOOST_FOREACH(const CTxDSIn& s, v.sev){
            if(s.scriptSig == newVin.scriptSig) {
                LogPrint("Luxsend", "CLuxsendPool::AddScriptSig - already exists \n");
                return false;
            }
        }
    }
    ...
    if(!SignatureValid(newVin.scriptSig, newVin)){
        LogPrint("Luxsend", "CLuxsendPool::AddScriptSig - Invalid Sig\n");
        return false;
    }

    LogPrint("Luxsend", "CLuxsendPool::AddScriptSig -- sig %s\n", newVin.ToString());

    BOOST_FOREACH(CTxIn& vin, finalTransaction.vin){
        if(newVin.prevout == vin.prevout && vin.nSequence == newVin.nSequence){
            vin.scriptSig = newVin.scriptSig;
            vin.prevPubKey = newVin.prevPubKey;
            LogPrint("Luxsend", "CLuxsendPool::AddScriptSig -- adding to finalTransaction  %s\n", newVin.scriptSig.ToString().substr(0,24));
        }
    }
    for(unsigned int i = 0; i < entries.size(); i++){
        if(entries[i].AddSig(newVin)){
            LogPrint("Luxsend", "CLuxsendPool::AddScriptSig -- adding  %s\n", newVin.scriptSig.ToString().substr(0,24));
            return true;
        }
    }


    LogPrintf("CLuxsendPool::AddScriptSig -- Couldn't set sig!\n" );
    return false;
}

// check to make sure everything is signed
bool CLuxsendPool::SignaturesComplete(){
...
//
// Execute a Luxsend denomination via a Masternode.
// This is only ran from clients
//
void CLuxsendPool::SendLuxsendDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount){

    if(fMasterNode) {
        LogPrintf("CLuxsendPool::SendLuxsendDenominate() - Luxsend from a Masternode is not supported currently.\n");
        return;
    }
    ...
    if(!sessionFoundMasternode){
        LogPrintf("CLuxsendPool::SendLuxsendDenominate() - No Masternode has been selected yet.\n");
        UnlockCoins();
        SetNull();
        return;
    }
    ...
    if (!CheckDiskSpace()) {
        UnlockCoins();
        SetNull();
        fEnableLuxsend = false;

        LogPrintf("CLuxsendPool::SendLuxsendDenominate() - Not enough disk space, disabling Luxsend.\n");
        return;
    }

    UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);

    LogPrintf("CLuxsendPool::SendLuxsendDenominate() - Added transaction to pool.\n");
...
// Incoming message from Masternode updating the progress of Luxsend
//    newAccepted:  -1 mean's it'n not a "transaction accepted/not accepted" message, just a standard update
//                  0 means transaction was not accepted
//                  1 means transaction was accepted

bool CLuxsendPool::StatusUpdate(int newState, int newEntriesCount, int newAccepted, std::string& error, int newSessionID){
    if(fMasterNode) return false;
    ...
    if(newAccepted != -1) {
        ...
        if(newAccepted == 1 && newSessionID != 0) {
            ...
            LogPrintf("CLuxsendPool::StatusUpdate - set sessionID to %d\n", sessionID);
    ...
    if(newState == POOL_STATUS_ACCEPTING_ENTRIES){
        if(newAccepted == 1){
            LogPrintf("CLuxsendPool::StatusUpdate - entry accepted! \n");
            sessionFoundMasternode = true;
            //wait for other users. Masternode will report when ready
            UpdateState(POOL_STATUS_QUEUE);
        } else if (newAccepted == 0 && sessionID == 0 && !sessionFoundMasternode) {
            LogPrintf("CLuxsendPool::StatusUpdate - entry not accepted by Masternode \n");
            UnlockCoins();
            UpdateState(POOL_STATUS_ACCEPTING_ENTRIES);
...
//
bool CLuxsendPool::SignFinalTransaction(CTransaction& finalTransactionNew, CNode* node){
    if(fMasterNode) return false;

    finalTransaction = finalTransactionNew;
    LogPrintf("CLuxsendPool::SignFinalTransaction %s\n", finalTransaction.ToString());

    vector<CTxIn> sigs;

    //make sure my inputs/outputs are present, otherwise refuse to sign
    BOOST_FOREACH(const CLuxsendEntry e, entries) {
        BOOST_FOREACH(const CTxDSIn s, e.sev) {
            ...
            if(mine >= 0){ //might have to do this one input at a time?
                ...
                for(unsigned int i = 0; i < finalTransaction.vout.size(); i++){
                    BOOST_FOREACH(const CTxOut& o, e.vout) {
                        ...
                        if(Ftx == Otx){
                            //if(fDebug) LogPrintf("CLuxsendPool::SignFinalTransaction - foundOutputs = %d \n", foundOutputs);
                            foundOutputs++;
                ...
                if(foundOutputs < targetOuputs || nValue1 != nValue2) {
                    ...
                    LogPrintf("CLuxsendPool::Sign - My entries are not correct! Refusing to sign. %d entries %d target. \n", foundOutputs, targetOuputs);
                    UnlockCoins();
                    SetNull();
                    return false;
                }
...
void CLuxsendPool::NewBlock()
{
    LogPrint("Luxsend", "CLuxsendPool::NewBlock \n");

    //we we're processing lots of blocks, we'll just leave
    if(GetTime() - lastNewBlock < 10) return;
    lastNewBlock = GetTime();

    LuxsendPool.CheckTimeout();

}

// Luxsend transaction was completed (failed or successful)
void CLuxsendPool::CompletedTransaction(bool error, int errorID)
{
    if(fMasterNode) return;
...
void CLuxsendPool::ClearLastMessage()
{
    lastMessage = "";
}

//
// Passively run Luxsend in the background to anonymize funds based on the given configuration.
//
// This does NOT run by default for daemons, only for QT.
//
bool CLuxsendPool::DoAutomaticDenominating(bool fDryRun)
{

    if(!fEnableLuxsend) return false;
    ...
    if(pindexBest->nHeight - cachedLastSuccess < minBlockSpacing) {
        LogPrintf("CLuxsendPool::DoAutomaticDenominating - Last successful Luxsend action was too recent\n");
        strAutoDenomResult = _("Last successful Luxsend action was too recent.");
        return false;
    }

    if(mnodeman.size() == 0){
        LogPrint("Luxsend", "CLuxsendPool::DoAutomaticDenominating - No Masternodes detected\n");
    ...
    // should not be less than fees in Luxsend_COLLATERAL + few (lets say 5) smallest denoms
    CAmount nLowestDenom = Luxsend_COLLATERAL + LuxsendDenominations[LuxsendDenominations.size() - 1]*5;

    // if there are no DS collateral inputs yet
    if(!pwalletMain->HasCollateralInputs())
        // should have some additional amount for them
        nLowestDenom += Luxsend_COLLATERAL*4;

    CAmount nBalanceNeedsAnonymized = nAnonymizeLuxcoinAmount*COIN - pwalletMain->GetAnonymizedBalance();

    // if balanceNeedsAnonymized is more than pool max, take the pool max
    if(nBalanceNeedsAnonymized > Luxsend_POOL_MAX) nBalanceNeedsAnonymized = Luxsend_POOL_MAX;
    ...
    LogPrint("Luxsend", "DoAutomaticDenominating : nLowestDenom=%d, nBalanceNeedsAnonymized=%d\n", nLowestDenom, nBalanceNeedsAnonymized);

    // select coins that should be given to the pool
    if (!pwalletMain->SelectCoinsDark(nValueMin, nBalanceNeedsAnonymized, vCoins, nValueIn, 0, nLuxsendRounds))
    {
        nValueIn = 0;
        vCoins.clear();
    ...
    if(!sessionFoundMasternode){
        ...
        if(nUseQueue > 33){
            ...
            BOOST_FOREACH(CLuxsendQueue& dsq, vecLuxsendQueue){
                ...
                if (!pwalletMain->SelectCoinsByDenominations(dsq.nDenom, nValueMin, nBalanceNeedsAnonymized, vTempCoins, vTempCoins2, nValueIn, 0, nLuxsendRounds)){
...
bool CLuxsendPool::PrepareLuxsendDenominate()
{
    std::string strError = "";
    // Submit transaction to the pool if we get here
    // Try to use only inputs with the same number of rounds starting from lowest number of rounds possible
    for(int i = 0; i < nLuxsendRounds; i++) {
        strError = pwalletMain->PrepareLuxsendDenominate(i, i+1);
        LogPrintf("DoAutomaticDenominating : Running Luxsend denominate for %d rounds. Return '%s'\n", i, strError);
        if(strError == "") return true;
    }

    strError = pwalletMain->PrepareLuxsendDenominate(0, nLuxsendRounds);
    LogPrintf("DoAutomaticDenominating : Running Luxsend denominate for all rounds. Return '%s'\n", strError);
    if(strError == "") return true;
...
bool CLuxsendPool::SendRandomPaymentToSelf()
{
...
bool CLuxsendPool::MakeCollateralAmounts()
{
    CWalletTx wtx;
    int64_t nFeeRet = 0;
    std::string strFail = "";
    ...
    vecSend.push_back(make_pair(scriptCollateral, Luxsend_COLLATERAL*4));
...
bool CLuxsendPool::CreateDenominated(int64_t nTotalValue)
{
    CWalletTx wtx;
    int64_t nFeeRet = 0;
    std::string strFail = "";
    ...
    if(!pwalletMain->HasCollateralInputs()) {
        vecSend.push_back(make_pair(scriptCollateral, Luxsend_COLLATERAL*4));
        nValueLeft -= Luxsend_COLLATERAL*4;
    }

    // ****** Add denoms ************ /
    BOOST_REVERSE_FOREACH(int64_t v, LuxsendDenominations){
        int nOutputs = 0;

        // add each output up to 10 times until it can't be added again
        while(nValueLeft - v >= Luxsend_COLLATERAL && nOutputs <= 10) {
            CScript scriptDenom;
            CPubKey vchPubKey;
...
bool CLuxsendPool::IsCompatibleWithEntries(std::vector<CTxOut>& vout)
{
...
bool CLuxsendPool::IsCompatibleWithSession(int64_t nDenom, CTransaction txCollateral,  std::string& strReason)
{
    if(nDenom == 0) return false;

    LogPrintf("CLuxsendPool::IsCompatibleWithSession - sessionDenom %d sessionUsers %d\n", sessionDenom, sessionUsers);

    if (!unitTest && !IsCollateralValid(txCollateral)){
        LogPrint("Luxsend", "CLuxsendPool::IsCompatibleWithSession - collateral not valid!\n");
    ...
    if((state != POOL_STATUS_ACCEPTING_ENTRIES && state != POOL_STATUS_QUEUE) || sessionUsers >= GetMaxPoolTransactions()){
        ...
        LogPrintf("CLuxsendPool::IsCompatibleWithSession - incompatible mode, return false %d %d\n", state != POOL_STATUS_ACCEPTING_ENTRIES, sessionUsers >= GetMaxPoolTransactions());
        return false;
    }
    ...
    LogPrintf("CLuxsendPool::IsCompatibleWithSession - compatible\n");
...
void CLuxsendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
...
int CLuxsendPool::GetDenominations(const std::vector<CTxDSOut>& vout){
...
int CLuxsendPool::GetDenominations(const std::vector<CTxOut>& vout, bool fSingleRandomDenom){
    std::vector<pair<int64_t, int> > denomUsed;

    // make a list of denominations, with zero uses
    BOOST_FOREACH(int64_t d, LuxsendDenominations)
...
int CLuxsendPool::GetDenominationsByAmounts(std::vector<int64_t>& vecAmount){
    CScript e = CScript();
...
int CLuxsendPool::GetDenominationsByAmount(int64_t nAmount, int nDenomTarget){
    CScript e = CScript();
...
std::string CLuxsendPool::GetMessageByID(int messageID) {
    switch (messageID) {
    ...
    case ERR_MAXIMUM: return _("Value more than Luxsend pool maximum allows.");
    ...
    case ERR_RECENT: return _("Last Luxsend was too recent.");
...
bool CLuxsendSigner::IsVinAssociatedWithPubkey(CTxIn& vin, CPubKey& pubkey){
    CScript payee2;
...
bool CLuxsendSigner::SetKey(std::string strSecret, std::string& errorMessage, CKey& key, CPubKey& pubkey){
    CLuxcoinSecret vchSecret;
...
bool CLuxsendSigner::SignMessage(std::string strMessage, std::string& errorMessage, vector<unsigned char>& vchSig, CKey key)
{
...
bool CLuxsendSigner::VerifyMessage(CPubKey pubkey, vector<unsigned char>& vchSig, std::string strMessage, std::string& errorMessage)
{
    CHashWriter ss(SER_GETHASH, 0);
    ss << strMessageMagic;
    ss << strMessage;
    ...
    if (fDebug && (pubkey2.GetID() != pubkey.GetID()))
        LogPrintf("CLuxsendSigner::VerifyMessage -- keys don't match: %s %s\n", pubkey2.GetID().ToString(), pubkey.GetID().ToString());

    return (pubkey2.GetID() == pubkey.GetID());
}

bool CLuxsendQueue::Sign()
{
    if(!fMasterNode) return false;
    ...
    if(!LuxsendSigner.SetKey(strMasterNodePrivKey, errorMessage, key2, pubkey2))
    {
        LogPrintf("CLuxsendQueue():Relay - ERROR: Invalid Masternodeprivkey: '%s'\n", errorMessage);
        return false;
    }

    if(!LuxsendSigner.SignMessage(strMessage, errorMessage, vchSig, key2)) {
        LogPrintf("CLuxsendQueue():Relay - Sign message failed");
        return false;
    }

    if(!LuxsendSigner.VerifyMessage(pubkey2, vchSig, strMessage, errorMessage)) {
        LogPrintf("CLuxsendQueue():Relay - Verify message failed");
        return false;
    }
...
bool CLuxsendQueue::CheckSignature()
{
    ...
    if(pmn != NULL)
    {
        std::string strMessage = vin.ToString() + boost::lexical_cast<std::string>(nDenom) + boost::lexical_cast<std::string>(time) + boost::lexical_cast<std::string>(ready);
        std::string errorMessage = "";
        if(!LuxsendSigner.VerifyMessage(pmn->pubkey2, vchSig, strMessage, errorMessage)){
...
void CLuxsendPool::RelayFinalTransaction(const int sessionID, const CTransaction& txNew)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH(CNode* pnode, vNodes)
    {
...
void CLuxsendPool::RelayIn(const std::vector<CTxDSIn>& vin, const int64_t& nAmount, const CTransaction& txCollateral, const std::vector<CTxDSOut>& vout)
{
    if(!pSubmittedToMasternode) return;
...
void CLuxsendPool::RelayStatus(const int sessionID, const int newState, const int newEntriesCount, const int newAccepted, const std::string error)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH(CNode* pnode, vNodes)
...
void CLuxsendPool::RelayCompletedTransaction(const int sessionID, const bool error, const std::string errorMessage)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH(CNode* pnode, vNodes)
...
void ThreadCheckLuxsendPool()
{
    if(fLiteMode) return; //disable all Luxsend/Masternode related functionality

    // Make this thread recognisable as the wallet flushing thread
    RenameThread("luxcoin-Luxsend");
    ...
    {
        MilliSleep(1000);
        //LogPrintf("ThreadCheckLuxsendPool::check timeout\n");

        // try to sync from all available nodes, one step at a time
        //masternodeSync.Process();


        if(LuxsendPool.IsBlockchainSynced()) {
            ...
            LuxsendPool.CheckTimeout();
            LuxsendPool.CheckForCompleteQueue();

            if(LuxsendPool.GetState() == POOL_STATUS_IDLE && c % 15 == 0){
                LuxsendPool.DoAutomaticDenominating();
            }
        }
    }
}
