...
class CTxIn;
class CLuxsendPool;
class CLuxsendSigner;
class CMasterNodeVote;
class CLuxcoinAddress;
class CLuxsendQueue;
...
#define Luxsend_QUEUE_TIMEOUT                 30
#define Luxsend_SIGNING_TIMEOUT               15

// used for anonymous relaying of inputs/outputs/sigs
#define Luxsend_RELAY_IN                 1
#define Luxsend_RELAY_OUT                2
#define Luxsend_RELAY_SIG                3

extern CLuxsendPool LuxsendPool;
extern CLuxsendSigner LuxsendSigner;
...
// A clients transaction in the Luxsend pool
class CLuxsendEntry
{
public:
    bool isSet;
    ...
    /// Add entries to use for Luxsend
    ...
    bool IsExpired()
    {
        return (GetTime() - addedTime) > Luxsend_QUEUE_TIMEOUT;// 120 seconds
    }
};


/**
 * A currently inprogress Luxsend merge and denomination information
 */
class CLuxsendQueue
{
public:
    ...
    /// Is this Luxsend expired?
    bool IsExpired()
    {
        return (GetTime() - time) > Luxsend_QUEUE_TIMEOUT;// 120 seconds
    }
...
/** Helper class to store Luxsend transaction (tx) information.
 */
class CLuxsendBroadcastTx
{
public:
...
{
public:
    /// Is the inputs associated with this public key? (and there is 10000 LUX - checking if valid masternode)
...
/** Used to keep track of current status of Luxsend pool
 */
class CLuxsendPool
{
private:
    mutable CCriticalSection cs_Luxsend;

    std::vector<CLuxsendEntry> entries; // Masternode entries
...
public:
    ...
    CLuxsendPool()
    {
        /* Luxsend uses collateral addresses to trust parties entering the pool
    ...
    /** Process a Luxsend message using the Luxsend protocol
     * \param pfrom
     * \param strCommand lower case command string; valid values are:
     *        Command  | Description
     *        -------- | -----------------
     *        dsa      | Luxsend Acceptable
     *        dsc      | Luxsend Complete
     *        dsf      | Luxsend Final tx
     *        dsi      | Luxsend vIn
     *        dsq      | Luxsend Queue
     *        dss      | Luxsend Signal Final Tx
     *        dssu     | Luxsend status update
     *        dssub    | Luxsend Subscribe To
     * \param vRecv
     */
    void ProcessMessageLuxsend(CNode* pfrom, std::string& strCommand, CDataStream& vRecv);

    void InitCollateralAddress(){
        SetCollateralAddress(Params().LuxsendPoolDummyAddress());
    }
    ...
    void UpdateState(unsigned int newState)
    {
        if (fMasterNode && (newState == POOL_STATUS_ERROR || newState == POOL_STATUS_SUCCESS)){
            LogPrint("Luxsend", "CLuxsendPool::UpdateState() - Can't set state to ERROR or SUCCESS as a Masternode. \n");
            return;
        }

        LogPrintf("CLuxsendPool::UpdateState() == %d | %d \n", state, newState);
        if(state != newState){
    ...
    /// Get Masternode updates about the progress of Luxsend
